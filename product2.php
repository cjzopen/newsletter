...
<?= @$product2 ? '' : '<meta name="robots" content="noindex, nofollow"></head>' ?>
<?php @$product2 ? '' : exit(); ?>
	<title>【產品特寫】<?= @strip_tags($productTitle2) ?> - ARES 資通電腦電子報</title>
	<meta name="description" content="<?= @strip_tags($productDescription2) ?>">
    ...
    <meta property="og:title" content="【產品特寫】<?= @strip_tags($productTitle2) ?> - ARES資通電子報" />
    <meta property="og:description" content="<?= @strip_tags($productDescription2) ?>" />
...
<style>table{width: 100%;margin:1rem 0;}th{background-color:#8064A2;text-align:center;color:#f0f0f0;}td{background-color:#c5bbca;}</style>
<body>
...
<script>(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
            ...
            <h1 itemprop="name"><?= @strip_tags($productTitle2) ?></h1>
            <div itemprop="author" itemscope itemtype="http://schema.org/Person" class="author">
                <img src="<?= @$productAuthorImg2 ?>" alt="author <?= @$productAuthor2 ?>">
                <i>by</i>
                <span itemprop="name" class="gray-color"><?= @$productAuthor2 ?></span>
            </div>
        </div>
        <section>
            ...
            <article itemprop="articleBody">
                ...
                <h3>製程參數智能化</h3>
                <p>
                    製成參數／配方管理（RMS）在自動化生產過程中，讓生產從自動化提升到智動化是相當重要的核心系統。現有自動化生產設備透過 DCS 即時的將生產過程中的生產參數或配方收集回系統，但傳統設備監控與自動化控制並沒有將設備生產參數與原料、生產批、生產條件、品質資訊等資料勾稽並積累分析，回饋修正最佳生產條件或配方／參數。資通 ciMes RMS 主要補強自動化設備資訊收集監控外的配方管理功能，重點在於透過資料的積累、影響製程能力及品質的變異因數，藉由大資料平台及分析回饋的機制，有效提升製程與品質能力。
                </p>
                <figure itemprop="image" itemscope itemtype="http://schema.org/ImageObject">
